...
async fn estimator_task<T: Transport + Clone, P: Provider<T, Ethereum> + Send + Sync + Clone + 'static>(
    ...
    match client.simulate_txes(simulation_bundle, estimate_request.block.into(), Some(vec![tx_hash])).await {
        Ok(sim_result) => {
            let sim_duration = chrono::Local::now() - start_time;
            debug!("Simulation result received Gas used : {} CB : {}  {} {}", sim_result.gas_used, sim_result.coinbase_tip, sim_result.coinbase_diff, sim_duration);
            ...
            if let Some(tx_sim_result) = sim_result.find_tx(tx_hash) {
                ...
                if let Some(access_list) = tx_sim_result.access_list.clone() {
                    ...
                    if gas_cost < profit_eth {
                        ...
                        let call_value: Option<U256> = if !token_in.is_weth() {
                            Some(tips)
                        } else {
                            None
                        };
                        ...
                        match compose_channel_tx.send(sign_request).await {
                            Ok(_) => { info!("Simulated bundle broadcast to flashbots") }
                            Err(e) => { error!("{}",e) }
                        }
                        ...
                        info!(" +++ Simulation successful. {:#32x} Cost {} Profit {} ProfitEth {} Tips {} {} {} {}", tx_hash, gas_cost_f64, profit_f64, profit_eth_f64, tips_f64, tx_sim_result, swap, sim_duration )
                    } else {
