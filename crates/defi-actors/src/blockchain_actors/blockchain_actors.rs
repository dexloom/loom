...

{
    ...
    pub async fn initialize_signers_with_encrypted_key(&mut self, key: Vec<u8>) -> Result<&mut Self> {
        self.actor_manager.start(InitializeSignersActor::new_from_encrypted_key(key).with_signers(self.signers.clone()).on_bc(&self.bc)).await?;
        Ok(self)
    }
    ...
    pub async fn with_encoder(&mut self, multicaller_address: Address) -> Result<&mut Self> {
        ...
        self.actor_manager.start(ArbSwapPathEncoderActor::new(multicaller_address).with_signers(self.signers.clone()).on_bc(&self.bc)).await?;
