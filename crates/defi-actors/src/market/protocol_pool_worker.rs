    ...
    */
    ...
    for factory_idx in 0..10 {
        match CurveProtocol::get_factory_address(client.clone(), factory_idx).await {
            Ok(factory_address) => match CurveProtocol::get_pool_count(client.clone(), factory_address).await {
                Ok(pool_count) => {
    ...
    P: Provider<T, N> + DebugProviderExt<T, N> + Send + Sync + Clone + 'static,
{
    pub fn new(client: P) -> Self {
        Self { client, market: None, market_state: None, _n: PhantomData::default(), _t: PhantomData::default() }
    }
