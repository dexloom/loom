...
impl UniswapV3StateReader {
    pub fn factory<DBErr>(db: &dyn DatabaseRef<Error = DBErr>, env: Env, pool: Address) -> eyre::Result<Address> {
    ...
    pub fn token0<DBErr>(db: &dyn DatabaseRef<Error = DBErr>, env: Env, pool: Address) -> eyre::Result<Address> {
        let call_data_result =
    ...
    pub fn token1<DB: DatabaseRef + ?Sized>(db: &DB, env: Env, pool: Address) -> eyre::Result<Address> {
        let call_data_result =
    ...
    pub fn fee<DB: DatabaseRef + ?Sized>(db: &DB, env: Env, pool: Address) -> eyre::Result<U24> {
        let call_data_result =
    ...
    pub fn tick_spacing<DB: DatabaseRef + ?Sized>(db: &DB, env: Env, pool: Address) -> eyre::Result<u32> {
        let call_data_result =
