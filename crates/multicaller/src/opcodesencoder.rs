...
use defi_types::{Opcode, Opcodes, OpcodeType};
...
pub trait OpcodesEncoder {
    fn pack_do_calls(opcodes: &Opcodes) -> Result<Bytes>;
    fn pack_do_calls_data(opcode: &Opcodes) -> Result<Bytes>;
}
...
impl OpcodesEncoder for OpcodesEncoderV2 {
    ...
    fn pack_do_calls_data(opcodes: &Opcodes) -> Result<Bytes> {
...
mod test {
    ...
    fn test() {
        ...
        let mut opcode = Opcode::new_internal_call(&buf);
