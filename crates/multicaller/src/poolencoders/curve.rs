...
impl CurveSwapEncoder {
    ...
    pub fn encode_swap_in_amount_provided(token_from_address: Address, token_to_address: Address, amount_in: SwapAmountType, swap_opcodes: &mut Opcodes, cur_pool: &PoolWrapper, next_pool: Option<&PoolWrapper>, multicaller: Address) -> Result<()> {
        ...
        match amount_in {
            ...
            SwapAmountType::Stack0 => {
                if in_native {
                    ...
                    let mut approve_opcode = Opcode::new_call(token_from_address, &EncoderHelper::encode_erc20_approve(cur_pool.get_address(), U256::ZERO));
            ...
            SwapAmountType::Balance(addr) => {
                let mut balance_opcode = Opcode::new_static_call(token_from_address, &EncoderHelper::encode_erc20_balance_of(addr));
        ...
        if out_native {
            let mut weth_deposit_opcode = Opcode::new_call_with_value(token_to_address, &EncoderHelper::encode_weth_deposit(), U256::ZERO);
        ...
        if let Some(next_pool) = next_pool {
            if Self::need_balance(cur_pool.get_address()) {
                let mut balance_opcode = Opcode::new_static_call(token_to_address, &EncoderHelper::encode_erc20_balance_of(multicaller));
                balance_opcode.set_return_stack(true, 0, 0x0, 0x20);
            ...
            match next_pool.get_encoder().preswap_requirement() {
                PreswapRequirement::Transfer(addr) => {
                    let mut transfer_opcode = Opcode::new_call(token_to_address, &EncoderHelper::encode_erc20_transfer(addr, U256::ZERO));
