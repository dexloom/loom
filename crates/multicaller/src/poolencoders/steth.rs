        ...
        if token_from_address == *WETH_ADDRESS && token_to_address == *STETH_ADDRESS {
            match amount_in {
                SwapAmountType::Set(amount) => {
                    ...
                    let swap_opcode = MulticallerCall::new_call_with_value(pool_address,
                ...
                SwapAmountType::Balance(addr) => {
                    let mut weth_balance_opcode = MulticallerCall::new_static_call(token_from_address, &EncoderHelper::encode_erc20_balance_of(addr));
            ...
            if next_pool.is_some() {
                let mut steth_balance_opcode = MulticallerCall::new_static_call(token_to_address, &EncoderHelper::encode_erc20_balance_of(multicaller));
