...
use quote::quote;
use syn::{Data, DeriveInput, Field, Fields, parse_macro_input, PathArguments, Type};
...
pub fn derive_access(input: TokenStream) -> TokenStream {
    ...
    let traits = access_fields.into_iter().map(|field| {
        let field_name = field.ident.expect("Field needs name");
        let field_type = match field.ty.clone() {
            Type::Path(type_path) => {
                match &type_path.path.segments[0].arguments.clone() {
