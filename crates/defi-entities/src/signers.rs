...
impl TxSigner {
    ...
    pub async fn sign(&self, tx_req: TransactionRequest) -> Result<(TxHash, Bytes)> {
        let mut typed_tx =
            tx_req.build_typed_tx().map_err(|_| eyre!("CANNOT_BUILD_TX"))?.eip1559().ok_or_eyre("CANNOT_BUILD_EIP1559")?.clone();
    ...
    pub fn sign_sync(&self, tx_req: TransactionRequest) -> Result<(TxHash, Bytes)> {
        let mut typed_tx =
            tx_req.build_unsigned().map_err(|_| eyre!("CANNOT_BUILD_UNSIGNED"))?.eip1559().ok_or_eyre("NOT_EIP_1599")?.clone();
