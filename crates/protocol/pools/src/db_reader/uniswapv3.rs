...
mod test {
    ...
    use loom_defi_entities::{MarketState, Pool};
    use loom_evm_db::LoomDBType;
    ...
    async fn test_reader() -> Result<()> {
        let _ = env_logger::try_init_from_env(env_logger::Env::default().default_filter_or(
            "info,loom_defi_entities::required_state=off,loom_defi_types::state_update=off,alloy_rpc_client::call=off,tungstenite=off",
        ));
