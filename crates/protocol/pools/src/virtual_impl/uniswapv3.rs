...
use eyre::eyre;
use loom_protocol_uniswap_v3_math::tick_math::{MAX_SQRT_RATIO, MAX_TICK, MIN_SQRT_RATIO, MIN_TICK};

use crate::db_reader::UniswapV3DBReader;
use crate::virtual_impl::tick_provider::TickProviderLoomDB;
use crate::UniswapV3Pool;
use loom_defi_entities::Pool;
...
impl UniswapV3PoolVirtual {
    pub fn simulate_swap_in_amount(db: &LoomDBType, pool: &UniswapV3Pool, token_in: Address, amount_in: U256) -> eyre::Result<U256> {
        ...
        while current_state.amount_specified_remaining != I256::ZERO && current_state.sqrt_price_x_96 != sqrt_price_limit_x_96 {
            ...
            (step.tick_next, step.initialized) = loom_protocol_uniswap_v3_math::tick_bitmap::next_initialized_tick_within_one_word(
                &tick_provider,
                current_state.tick,
                tick_spacing as i32,
                zero_for_one,
            ...
            step.sqrt_price_next_x96 = loom_protocol_uniswap_v3_math::tick_math::get_sqrt_ratio_at_tick(step.tick_next)?;
            ...
            (current_state.sqrt_price_x_96, step.amount_in, step.amount_out, step.fee_amount) =
                loom_protocol_uniswap_v3_math::swap_math::compute_swap_step(
            ...
            if current_state.sqrt_price_x_96 == step.sqrt_price_next_x96 {
                ...
                current_state.tick = loom_protocol_uniswap_v3_math::tick_math::get_tick_at_sqrt_ratio(current_state.sqrt_price_x_96)?;
            }
        }
...
mod test {
    ...
    use loom_protocol_uniswap_v3_math::full_math::mul_div_rounding_up;
