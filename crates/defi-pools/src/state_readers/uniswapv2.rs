...
impl UniswapV2StateReader {
    pub fn factory(db: &LoomInMemoryDB, env: Env, pool: Address) -> Result<Address> {
        ...
        let call_data_result = evm_call(db, env, pool, IUniswapV2Pair::IUniswapV2PairCalls::factory(IUniswapV2Pair::factoryCall {}).abi_encode())?.0;
        let call_return = IUniswapV2Pair::factoryCall::abi_decode_returns(&call_data_result, false)?;
        Ok(call_return._0)
    }

    pub fn token0(db: &LoomInMemoryDB, env: Env, pool: Address) -> Result<Address> {
        let call_data_result = evm_call(db, env, pool, IUniswapV2Pair::IUniswapV2PairCalls::token0(IUniswapV2Pair::token0Call {}).abi_encode())?.0;
        let call_return = IUniswapV2Pair::token0Call::abi_decode_returns(&call_data_result, false)?;
        Ok(call_return._0)
    }

    pub fn token1(db: &LoomInMemoryDB, env: Env, pool: Address) -> Result<Address> {
        let call_data_result = evm_call(db, env, pool, IUniswapV2Pair::IUniswapV2PairCalls::token1(IUniswapV2Pair::token1Call {}).abi_encode())?.0;
     ...
     */
    pub fn get_reserves(db: &LoomInMemoryDB, env: Env, pool: Address) -> Result<(U256, U256)> {
        let call_data_result = evm_call(db, env, pool, IUniswapV2Pair::IUniswapV2PairCalls::getReserves(IUniswapV2Pair::getReservesCall {}).abi_encode())?.0;
